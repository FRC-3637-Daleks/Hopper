...
public:
  ClimbSimulation(Climb &climb)
      : m_motorSim{climb.m_climbMotor.GetSimCollection()},
        m_climbModel{ClimbConstants::kWindowMotor,
...
Climb::Climb() : m_sim_state{new ClimbSimulation(*this)} {
  ...
  m_climbMotor.SetSelectedSensorPosition(0, ClimbConstants::kPIDLoopIdx,
                                         ClimbConstants::kTimeoutMs);
  m_climbMotor.ConfigSelectedFeedbackSensor(
      ctre::phoenix::motorcontrol::FeedbackDevice::Analog /*idk if correct*/,
      ClimbConstants::kPIDLoopIdx, ClimbConstants::kTimeoutMs);
  ...
  frc::DataLogManager::Log(
      fmt::format("Finished initializing climb subsystem."));
}
...
void Climb::SimulationPeriodic() {
  ...
  m_sim_state->m_motorSim.SetBusVoltage(
      frc::RobotController::GetBatteryVoltage().value());
  m_sim_state->m_climbModel.SetInputVoltage(
      m_sim_state->m_motorSim.GetMotorOutputLeadVoltage() * 1_V);

  m_sim_state->m_climbModel.Update(20_ms);

  m_sim_state->m_motorSim.SetAnalogPosition(
      m_sim_state->m_climbModel.GetPosition() /
      ClimbConstants::kMotorEncoderDistancePerCount / 1_m);
  m_sim_state->m_motorSim.SetAnalogVelocity(
      m_sim_state->m_climbModel.GetVelocity() /
      ClimbConstants::kMotorEncoderDistancePerCount / 1_mps * 100_ms / 1_ms);
  ...
  frc::SmartDashboard::PutNumber(
      "Climb/sim/Climb Position (m)",
      m_sim_state->m_climbModel.GetPosition().value());
  frc::SmartDashboard::PutNumber(
      "Climb/sim/Climb Velocity (mps)",
      m_sim_state->m_climbModel.GetVelocity().value());
}
...
frc2::CommandPtr Climb::ExtendClimb() {
  ...
  return frc2::cmd::StartEnd([this] { m_climbMotor.Set(1); },
                             [this]() { m_climbMotor.Set(0); }, {this});
}
