...
void RobotContainer::ConfigureBindings() {
  ...
  //     .WhileTrue(m_swerve.SwerveCommandFieldRelative(fwd, strafe, rot));
      ...
      auto BlueSourcePath = pathplanner::AutoBuilder::pathfindToPose(OperatorConstants::kBlueSourcePickUp,  constraints, 0_mps, 0_m);
      auto RedSourcePath = pathplanner::AutoBuilder::pathfindToPose(OperatorConstants::kRedSourcePickUp,  constraints, 0_mps, 0_m);
      auto BlueAmpShotPath = pathplanner::AutoBuilder::pathfindToPose(OperatorConstants::kBlueAmpShot,  constraints, 0_mps, 0_m);
      auto RedAmpShotPath = pathplanner::AutoBuilder::pathfindToPose(OperatorConstants::kRedAmpShot,  constraints, 0_mps, 0_m);

      m_SourcePath = frc2::cmd::Either(std::move(RedSourcePath), std::move(BlueSourcePath), checkRed);
      m_AmpShotPath = frc2::cmd::Either(std::move(RedAmpShotPath), std::move(BlueAmpShotPath), checkRed);
